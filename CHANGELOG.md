...
_Minor release promoted from `v3.7.0-rc.1`. Adds the read-only `evo status` one-screen view and escalating repeated-signal advice; makes the TypeScript renderer the single statusline renderer (older wrapper installs auto-migrate on the next `evo install-statusline`); and consolidates the growth curve onto the Ideal State Gauge, the hint dictionary into one committed JSON asset, and multi-window live-state onto a seq+pid generation protocol. Backward compatible: older profiles/DBs and statusline installs keep working untouched._
...
- **Repeated-signal advice now escalates through three levels instead of going quiet.** Previously, once the same coaching signal fired three times for the same target, EvoPet suppressed the advice and fell back to a generic rotating tip — so guidance disappeared exactly when you were stuck repeating the same mistake. Now the same signal escalates: the 1st–2nd fire keeps the normal phrasing, the 3rd–4th switches to a stronger “this keeps happening” (`続いてるよ` / `繰り返されています`) frame, and the 5th–6th uses the strongest stop-and-regroup phrasing; only at the 7th+ fire does it fall back to rotating tips (anti-nagging). Praise signals (good structure / first-pass success / improving trend) do **not** escalate — a “this keeps happening” frame makes no sense for praise — and keep the previous suppress-at-third behavior. Escalation is tracked per target, so a different file or symbol starts back at level 1, and a rotation reset zeroes it. The live-state payload gains an additive `adviceEscalationLevel` field (0 = rotating tip / suppressed, 1–3 = rendered advice level); existing consumers are unaffected.

### Changed
- **The TypeScript renderer is now the single statusline renderer (`evo install-statusline` wires `evo statusline --full`).** `evo install-statusline` no longer deploys the 1090-line Python `statusline.py`; it points Claude Code's `settings.json` `statusLine` at `evo statusline --full`, which renders the **complete** statusline — the token/model/context line **and** the EvoPet block — from one process. The new `--full` flag adds the token line to the TS renderer (the default `evo statusline`, EvoPet-only, is unchanged so the split-wrapper construction keeps working). The TS renderer reached parity with `statusline.py`: it now renders the 10s–5min **stale/dimmed** layout with the `(待機中)` marker (EvoPet no longer vanishes during a long tool call), the always-on essentials placeholders (`評価 —` / `指示の質: 計測中` / `育成度 -`), the proxy-path mood line, and the `待機中` quiet placeholder for a bound session with no data of its own; the grade-C label is aligned to `○ C もう一息`. `readFreshestLiveState()` (B2) now resolves the freshest generation across the shared sinks in the sessionless path. **Backward compatible**: a setup installed by an older version (`python "…/base_statusline.py"`) keeps working untouched until you re-run `evo install-statusline`, which migrates it to the TS wiring and cleans up the orphaned script. `evo doctor` recognizes both constructions, and the #116 content-based wrapper guard still protects hand-built wrapper setups (the installer exempts its own `evo statusline --full` command so re-running stays idempotent). `statusline.py` stays in the package as the compat/parity reference and is marked legacy; Python is no longer required for a fresh install. Migration note: README "Quick install".
- **Growth curve consolidated: the Ideal State Gauge (ISG) is now the single authoritative stage curve.** The superseded v3.0 cumulative-EXP stage curve (egg ≥0 / sprout ≥120 / buddy ≥320 / wizard ≥720 / legend ≥1400 EXP) — dead code since v3.1 switched stage selection to ISG — has been removed. ISG thresholds and penalties are unchanged (egg <25 / sprout 25 / buddy 45 / wizard 65 / legend 82; no re-tuning). **Backward compatible with existing data**: `totalBondExp` is still stored, accumulated and displayed exactly as before (累計 EXP) — it just never drives stage; profiles/DBs written by any older version load unchanged, keep their recorded stage on load, and re-derive it from ISG on the next finalized episode (the behavior since v3.1). Mapping for pre-v3.1 users: a stage that was EXP-promoted (e.g. legend from ≥1400 EXP) is not reset on upgrade, but converges to the ISG band matching your sustained prompt quality as new episodes finalize.
- **Hint dictionary unified into a single committed asset (`src/data/statusline-dict.json`).** The hand-curated Japanese EvoPet dictionary (the 5 mood-comment pools and the tips rotation, including the weekly AUTO-synced official-docs groups) previously lived in two hand-maintained copies — the Python literals in `statusline.py` and a "do not edit by hand" TypeScript extraction (`src/cli/statusline-data-raw.ts`) with no regeneration script, so the copies drifted. Now the JSON is the single source of truth: the TS renderer imports it directly, and `statusline.py` embeds the same JSON text verbatim in a generated section maintained by the new `scripts/gen-statusline-dict.mjs` (`--check` mode is enforced by the test suite, so drift fails CI). The Python renderer's effective dictionary is byte-identical to before; the TS renderer's fallback tip pool now matches the Python one exactly (it gains the entries the drifted copy was missing). `scripts/sync-claude-docs.mjs` now updates the JSON's auto groups (and regenerates the `statusline.py` section) instead of rewriting Python literals; a content-identical weekly run no longer bumps the `fetched` date, so no-change syncs produce no diff. Deployed statusline files remain single-file and old installs are unaffected.

### Fixed
- **Consistent live-state across the three statusline sinks (multi-window robustness).** EvoPet writes each statusline generation to three files — the project `.evo/live-state.json`, the shared `~/.claude/.evo-live.json`, and the per-session `.evo/sessions/<id>.json`. Those writes were individually atomic but not stamped as a set, so a reader crossing sinks could mix a newer generation with an older one, and the background session-file cleanup could unlink by mtime a file a live window was still writing. Each generation is now stamped with a monotonic per-writer sequence number, the writer's pid, and a write timestamp (a single serialization makes the JSON byte-identical on all three sinks); each sink is written independently so one failing file can no longer block the others; and the cleanup pass consults the session-owner registry and skips any file a live process still owns (dead or aged-out files are reclaimed as before, and a probe error conservatively keeps the file). These races are most likely to bite with several Claude Code windows open in the same folder.

### Internal
- B2 adds a documented, parse-tolerant `readFreshestLiveState()` reader helper (`src/proxy/liveStateReader.ts`) that ranks candidate sinks by live pid, then per-writer sequence (clock-step-proof), then write time, with the earlier path winning ties; the statusline consumes it through the TS renderer's sessionless path. Covered by 34+ new/updated tests (sequence monotonicity, protocol stamping + shadow-proofing, byte-equal fan-out, per-sink error isolation, the GC live/dead/orphan/probe-error matrix, and end-to-end mixed-generation reads). Design note: `docs/ai/b2-sink-consistency.md`.

## v3.7.0-rc.1 (2026-07-22)
