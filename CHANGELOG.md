...
- Before/after examples and advice are truncated by meaning instead of a fixed byte offset: width-aware (CJK counts as 2 columns), whole-string filesystem paths collapse to their basename, overflow is cut at the last clause/word boundary, and elided headlines end with `→ 続きは \`evo advice\`` instead of a mid-glyph cut. Signal example paths are shortened to a repo-relative basename at generation time. As a final safety net on top of the per-field truncation, both renderers hard-cap the visible length of each emitted line, so a pathological payload (e.g. a crafted 200KB advice or nickname) can never flood the statusline — it is cut hard with the `evo advice` pointer.
