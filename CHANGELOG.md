...
### Fixed
- Eliminated the remaining `SQLITE_BUSY: database is locked` crashes under concurrent same-cwd proxies (adding `busy_timeout` alone did not fix them). The real cause was a SQLite upgrade-deadlock: several write paths ran as DEFERRED transactions (or read-then-write auto-commit paths) that took a read snapshot and then upgraded to a write — under concurrency the upgrade fails **immediately** with `SQLITE_BUSY` (the busy handler is bypassed for deadlocks, so the timeout never applies). Every write transaction now BEGINs `IMMEDIATE`, taking the write lock up front so concurrent writers serialize instead of deadlocking; the multi-write entry points (`createEpisode`, `finishEpisode`, token calibration) are wrapped in a single atomic transaction; a bounded `SQLITE_BUSY` retry (5 attempts, linear backoff) backstops the top-level writes; and the CLI now logs the failing statement stack and error code at debug so future DB errors are diagnosable.
...
- Fixed a `SQLITE_BUSY` crash when two proxies open a brand-new database at the same instant (~2/25 two-way fresh-db launches). The connection's `PRAGMA journal_mode = WAL` takes a brief exclusive lock and, unlike ordinary writes, can return `SQLITE_BUSY` **immediately without invoking the busy handler**, so `busy_timeout` did not cover it. The WAL switch is now wrapped in the same bounded retry as the other writes — on a retry the winning proxy has finished and the database is already in WAL, so the pragma returns without contention. This closes the third and last of the concurrent-first-open crash tiers (config atomic writes, the stats/migration races, and now the WAL switch).
- Fixed `SQLITE_BUSY: database is locked` crashes under concurrent same-cwd proxies (~1/10 runs with 3 sessions launching together, one exiting `1`). The database opened in WAL mode but never set `busy_timeout`, so a second concurrent writer errored immediately instead of waiting its short turn; the connection now sets `busy_timeout = 5000ms`. The opportunistic `wal_checkpoint(TRUNCATE)` during compaction is also now guarded — an `SQLITE_BUSY` there (racing another proxy's finalize writes) is logged and skipped instead of failing the session; the WAL is truncated on a later run.
