...
- 過去履歴をもとに、その人の癖に寄せた節約予測と改善提案を出す
- 良い頼み方が刺さった時に、EXP と称賛で手応えを返す
- TS / JS / Python は関数単位の差分も追う
- ローカル学習結果を `.evo` に保存し、他 PC へ持ち運べる

## 普段の使い方

普段は `evo run ...` のような専用コマンドを毎回打つ必要はありません。  
セットアップ後は、いつも通り `codex` や `claude` を起動するだけです。

つまり、日常の体験はこうです。

1. `codex` または `claude` を起動する
2. ふつうに依頼する
3. Evo が裏で記録する
4. 必要な時だけ、その場で短い改善提案や称賛が飛んでくる

`evo stats` や `evo explain` は、あとから深掘りしたい人向けの補助コマンドです。  
普段の利用では、まずそこを見なくても動く設計です。

## どういうアドバイスが飛んでくるか

Evo は常に同じ文を出すのではなく、状況に応じて出し分けます。

- 具体化不足: 「関数名や対象ファイルを 1 つ足すと刺さりやすい」
- 構造不足: 「箇条書きと完了条件を 2 行足すと通りやすい」
- テスト不足: 「成功条件を 1 行足すとやり直しを減らしやすい」
- 探索散乱: 「次は対象ファイルを 1 つに絞る」
- edit loop: 「現状 / 期待 / NG 条件に分けて渡し直す」
- 上手く進んだ時: 「今の頼み方は強い」「この絞り方はかなり良い」などの称賛
...
つまり、ただの監視ログではなく、エンジニアが「あと一言足せば浮くのか」と思えるような攻めの UI を目指しています。
...
│ 次にやると刺さりやすいこと: 次は 関数名 か 対象ファイル名 を 1 個足す
...
以下は「設定を変えたい」「履歴を見たい」「後から深掘りしたい」時だけ使うものです。
